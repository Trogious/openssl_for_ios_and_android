...
LIB_VERSION="openssh-3_2_3"
LIB_NAME="openssh-8.1p1"
LIB_DEST_DIR="${pwd_path}/../output/android/curl-universal"

echo "https://download.samba.org/pub/openssh/${LIB_NAME}.tar.gz"
...
[ -f "${LIB_NAME}.tar.gz" ] || curl -LO https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/${LIB_NAME}.tar.gz >${LIB_NAME}.tar.gz
...
function configure_make() {
    ...
    PREFIX_DIR="${pwd_path}/../output/android/openssh-${ABI}"
    if [ -d "${PREFIX_DIR}" ]; then
        rm -fr "${PREFIX_DIR}"
    fi
    mkdir -p "${PREFIX_DIR}"

    OUTPUT_ROOT=${TOOLS_ROOT}/../output/android/openssh-${ABI}
    mkdir -p ${OUTPUT_ROOT}/log

    set_android_toolchain "openssh" "${ARCH}" "${ANDROID_API}"
    set_android_cpu_feature "openssh" "${ARCH}" "${ANDROID_API}"
    ...
    if make -j$(get_cpu_count) ssh >>"${OUTPUT_ROOT}/log/${ABI}.log" 2>&1; then
